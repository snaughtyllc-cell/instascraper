...
app.get('/content', async (req, res) => {
    const {
          page = 1, limit = 24, sort = 'newest', tag, account,
          minViews, startDate, endDate, search, showArchived, contentType,
    } = req.query;
    ...
    let paramIdx = 1;
          ...
          if (contentType) {
                where.push(`COALESCE(posts.content_type, ct.content_type) = $${paramIdx++}`);
                params.push(contentType);
          }
    if (tag) {
          where.push(`tag = $${paramIdx++}`);
          params.push(tag);
    }
    if (account) {
          where.push(`posts.account_handle = $${paramIdx++}`);
          params.push(account);
    }
    if (minViews) {
          where.push(`view_count >= $${paramIdx++}`);
          params.push(Number(minViews));
    }
    if (startDate) {
          where.push(`posted_at >= $${paramIdx++}`);
    ...
    if (search) {
          where.push(`caption ILIKE $${paramIdx++}`);
          params.push(`%${search}%`);
    }
    ...
    const offset = (Number(page) - 1) * Number(limit);
          ...
          const countResult = await pool.query(
                `SELECT COUNT(*) as count FROM posts ${joinClause} ${whereClause}`,
                params
              );
    const total = parseInt(countResult.rows[0].count);

          const postsResult = await pool.query(
                `SELECT posts.* FROM posts ${joinClause} ${whereClause} ORDER BY ${orderBy} LIMIT $${paramIdx++} OFFSET $${paramIdx++}`,
                [...params, Number(limit), offset]
              );

          const accountsResult = await pool.query(
                `SELECT DISTINCT account_handle FROM posts WHERE account_handle != ''`
              );
...
app.post('/creators/:handle/type', async (req, res) => {
    ...
    if (contentType) {
          await pool.query(
                  'INSERT INTO creator_types (account_handle, content_type) VALUES ($1, $2) ON CONFLICT (account_handle) DO UPDATE SET content_type = $2',
                  [req.params.handle, contentType]
                );
    } else {
            ...
            FROM posts p
                LEFT JOIN creator_types ct ON p.account_handle = ct.account_handle
                    WHERE p.account_handle != ''
                        GROUP BY p.account_handle, ct.content_type
...
app.post('/content/:id/archive', async (req, res) => {
    ...
    const result = await pool.query('UPDATE posts SET archived = $1 WHERE id = $2', [val, Number(req.params.id)]);
    if (result.rowCount === 0) return res.status(404).json({ error: 'Post not found' });
    res.json({ success: true });
});
...
app.get('/engagement/summary/:handle', async (req, res) => {
    ...
    const result = await pool.query(`
        SELECT shortcode, posted_at, like_count, comment_count, followers_at_scrape, er_percent, er_label
            FROM posts
                WHERE account_handle = $1 AND (archived = 0 OR archived IS NULL)
...
app.get('/engagement/export/:handle', async (req, res) => {
    const handle = req.params.handle;
    const result = await pool.query(`
        SELECT shortcode, posted_at, like_count, comment_count, view_count,
              followers_at_scrape, er_percent, er_label, post_url, caption
                  FROM posts
                      WHERE account_handle = $1 AND (archived = 0 OR archived IS NULL)
