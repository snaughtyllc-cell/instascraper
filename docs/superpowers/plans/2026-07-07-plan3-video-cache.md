...
> **↳ Codex-review (Round 1) shaped this plan.** Nine findings were incorporated (see the changes noted inline as `[CX-1..15]`); two were rejected with reasons recorded in `2026-07-07-plan3-video-cache-REVIEW-LOG.md` — the cross-process DB lease (over-engineered for a single-instance deploy; the URL-guarded UPDATE removes the real harm) and the removal of the 302 fallback (kept narrowly; it plays fresh-but-unswept videos and degrades to the shipped poster on expiry).

## Global Constraints

- **Backend tests:** `node --test` only, on extracted pure logic + fake `fs`/`fetch`/in-memory `better-sqlite3` — the exact style of `server/thumbnails.js` (dependency-injected: `deps.fs`, `deps.fetch`, `deps.db`, `deps.videoDir`, `deps.now`, `deps.inflight`). Do NOT boot Express or hit the network in tests. Use the `db.query(sql, params)`→sqlite adapter from `server/content-types-seed.test.js` for DB-touching helpers.
...
- **Freshness vs. retention are two different windows [CX-1, CX-2]:**
  - **Freshness** governs whether a signed `video_url` is worth attempting. A `'pending'` row was just re-scraped so its URL is fresh — always sweep it (mirrors the thumbnail sweep's documented pending-always rule). A legacy `NULL`-status row is only worth trying if `scraped_at` is recent (URL not yet expired) — gate it by `scraped_at >= freshnessCutoff`. **Freshness uses `scraped_at`, never `posted_at`.**
  - **Retention** bounds storage: only cache posts whose `posted_at` is within the last 30 days, so we don't spend a download on a re-scraped 45-day-old repost that prune would delete a day later. Gate the selector by `(posted_at >= retentionCutoff OR posted_at IS NULL)`. **Retention uses `posted_at`.**
...
- **Auth:** `GET /video/:id` sits behind `requireAuth` via `app.use('/video', requireAuth)` (exactly like `/thumb` at `index.js:107`), so both admin and — once Plan 2 lands — model sessions can stream. Not `requireAdmin`. The client is served by the **same Express app** (`express.static(clientBuild)` at `index.js:928`), so `<video src="/video/:id">` is **same-origin** and the session cookie flows automatically — no CORS/`crossorigin` needed [CX-12].
...
**Files:**
- Modify: `server/db.js` — add to the Postgres `ADD COLUMN IF NOT EXISTS` list (after `thumbnail_cache_status`/`thumbnail_cache_error` at `db.js:322-323`) and the SQLite `ADD COLUMN` fallback list (at `db.js:339-340`).
...
> **[CX-13] The test must exercise the REAL migration, not hand-written DDL.** A test that `CREATE TABLE`s its own `posts` and alters it passes even if `db.js` is never touched — it is vacuous. Instead, run the real `initDB()` and assert the three columns exist on the migrated `posts` table (a `SELECT` of them throws if the migration didn't add them). `initDB()` runs no Express and no network — it only executes DDL against the local better-sqlite3 DB (same call the boot check already makes), so this respects the Global Constraints.

- [ ] **Step 1: Write the failing test — against the real migration**
...
// Force sqlite mode (no DATABASE_URL) so initDB runs the SQLite migration branch
// against the local dev DB — this is the SAME path the boot check exercises.
delete process.env.DATABASE_URL;
const db = require('./db');

test('initDB adds video_cache_status/error/cached_at to the real posts table', async () => {
  await db.initDB();
  // Throws if the migration did NOT add these columns — this is the anti-vacuous guard.
  const r = await db.query('SELECT video_cache_status, video_cache_error, video_cached_at FROM posts LIMIT 0');
  assert.ok(r, 'select of the three video cache columns must not throw');
});
```

- [ ] **Step 2: Run → FAIL** (`cd server && node --test video-schema.test.js`) — the SELECT throws `no such column: video_cache_status` because `db.js` hasn't been changed yet. This failure is what proves the test is non-vacuous.

- [ ] **Step 3: Add the migrations to `db.js`** — in BOTH the Postgres `IF NOT EXISTS` list (after `db.js:323`) and the SQLite fallback `ADD COLUMN` list (after `db.js:340`), following the existing `thumbnail_cache_status`/`thumbnail_cache_error` lines exactly:
...
// SQLite fallback branch (near db.js:339) — same three without IF NOT EXISTS,
// matching how thumbnail_cache_status is handled there:
...
(Match the exact wrapping/error-swallowing the SQLite branch already uses for `thumbnail_cache_status` — the fallback branch tolerates "duplicate column" on re-run.)

- [ ] **Step 4: Run → PASS** (`cd server && node --test video-schema.test.js`). Also boot check: `cd server && node -e "delete process.env.DATABASE_URL; require('./db').initDB().then(()=>console.log('ok'))"` → `ok`.

- [ ] **Step 5: Commit** — `git commit -am "feat(video-cache): video_cache_status/error/cached_at columns (real-migration test)"`
...
- Produces:
  ...
  - `tempVideoPath(key, videoDir?) : string` → a unique temp path `<videoDir>/<key>.<pid>.<ts>.tmp`, mirroring the thumbnail temp-naming so concurrent writers never collide.
...
function tempVideoPath(key, videoDir = DEFAULT_VIDEO_DIR) {
  return path.join(videoDir, `${key}.${process.pid}.${Date.now()}.tmp`);
}
...
- Produces: `downloadVideo(post, deps?) : Promise<{ status: 'cached'|'expired'|'error'|'skipped', path?, error? }>` — dependency-injected (`deps.fs`, `deps.fetch`, `deps.videoDir`, `deps.inflight`), mirroring `downloadThumbnail` (`thumbnails.js:9-48`) with two video-specific changes:
  ...
  - **Streamed byte cap [CX-6]:** stream `res.body` to the temp file, incrementing a byte counter; if it exceeds `VIDEO_MAX_MB * 1024 * 1024`, destroy the stream, `unlinkSync` the temp, and return `skipped`. Never `res.buffer()` the whole file. If `content-length` is present and already over the cap, short-circuit to `skipped` before streaming.

  Behavior: return `cached` if the final file already exists on disk (stat > 0); `expired` on 403/404; `error` on other non-ok / stream failures; `skipped` if over the size cap (by header OR by streamed bytes); write to a unique temp then `renameSync` to the final path; `finally` delete the in-flight entry.

> The fake-`fs` in the test must model a writable stream so the streamed-cap path is exercised. Provide a `createWriteStream` that accumulates bytes and a `res.body` that is an async iterable / Node Readable of chunks. Keep it minimal — the point is to assert the four outcomes, not to reimplement streams faithfully. If modeling a real stream in the fake is awkward, the implementer may have `downloadVideo` accept an injected `deps.pipeBody(res, tmpPath, cap, fs)` seam that the test stubs — decide during implementation, but the size-cap-without-content-length behavior MUST be covered by a test either way.

- [ ] **Step 1: Write the failing tests (fake fs + fetch)** — cover all four outcomes AND the two risky paths [CX-14]:
...
// Minimal fake fs with a byte-counting write stream.
function fakeFs(existing = {}) {
  const files = { ...existing };
  return {
    _files: files,
    statSync: (f) => { if (files[f] == null) { const e = new Error('nope'); e.code = 'ENOENT'; throw e; } return { size: files[f] }; },
    mkdirSync: () => {},
    ...
    createWriteStream: (f) => {
      files[f] = 0;
      return {
        write: (chunk) => { files[f] += chunk.length; return true; },
        end: (cb) => { if (cb) cb(); },
        destroy: () => {},
        on: () => {},
      };
...
// A fetch whose body yields `chunks` of `chunkLen` bytes each.
function bodyFetch({ status = 200, headers = {}, chunks = 1, chunkLen = 1000 } = {}) {
  return async () => ({
    status, ok: status >= 200 && status < 300,
    headers: { get: (k) => headers[k.toLowerCase()] ?? null },
    body: (async function* () { for (let i = 0; i < chunks; i++) yield Buffer.alloc(chunkLen, 1); })(),
  });
}

test('downloadVideo streams the body to disk and returns cached', async () => {
  const fs = fakeFs();
  const r = await downloadVideo({ id: 7, video_url: 'http://x/v.mp4' },
    { fs, fetch: bodyFetch({ headers: { 'content-type': 'video/mp4' }, chunks: 3, chunkLen: 1000 }), videoDir: '/v', inflight: new Map() });
  assert.strictEqual(r.status, 'cached');
  assert.ok(fs._files['/v/7.mp4'] > 0);
});
test('downloadVideo returns expired on 403', async () => {
  const r = await downloadVideo({ id: 7, video_url: 'http://x' },
    { fs: fakeFs(), fetch: bodyFetch({ status: 403 }), videoDir: '/v', inflight: new Map() });
  assert.strictEqual(r.status, 'expired');
});
test('downloadVideo skips when content-length is over the cap', async () => {
  const big = String((VIDEO_MAX_MB + 5) * 1024 * 1024);
  const r = await downloadVideo({ id: 7, video_url: 'http://x' },
    { fs: fakeFs(), fetch: bodyFetch({ headers: { 'content-length': big, 'content-type': 'video/mp4' } }), videoDir: '/v', inflight: new Map() });
  assert.strictEqual(r.status, 'skipped');
});
test('downloadVideo skips a too-big body even with NO content-length header [CX-6]', async () => {
  // No content-length; stream more bytes than the cap → must abort as skipped.
  const overCapChunks = Math.ceil((VIDEO_MAX_MB * 1024 * 1024) / (1024 * 1024)) + 2;
  const r = await downloadVideo({ id: 7, video_url: 'http://x' },
    { fs: fakeFs(), fetch: bodyFetch({ headers: { 'content-type': 'video/mp4' }, chunks: overCapChunks, chunkLen: 1024 * 1024 }), videoDir: '/v', inflight: new Map() });
  assert.strictEqual(r.status, 'skipped');
});
test('downloadVideo returns cached without refetching if file exists', async () => {
  const fs = fakeFs({ '/v/7.mp4': 5 });
  let fetched = false;
  const r = await downloadVideo({ id: 7, video_url: 'http://x' },
    { fs, fetch: async () => { fetched = true; return {}; }, videoDir: '/v', inflight: new Map() });
  assert.strictEqual(r.status, 'cached');
...
test('downloadVideo dedups concurrent fetches of the same id+url [CX-5]', async () => {
  const fs = fakeFs();
  let calls = 0;
  const slow = async () => { calls++; await new Promise(r => setTimeout(r, 20)); return { status: 200, ok: true, headers: { get: () => null }, body: (async function* () { yield Buffer.alloc(10, 1); })() }; };
  const inflight = new Map();
  const post = { id: 7, video_url: 'http://x/v.mp4' };
  const [a, b] = await Promise.all([downloadVideo(post, { fs, fetch: slow, videoDir: '/v', inflight }), downloadVideo(post, { fs, fetch: slow, videoDir: '/v', inflight })]);
...
- [ ] **Step 3: Implement `downloadVideo`** in `videos.js` mirroring `downloadThumbnail`: stat the final file first (`cached` short-circuit); build `inflightKey = key + ':' + post.video_url`, dedup on it; fetch with UA + `timeout: 30000`; 403/404 → `expired`; other non-ok → `error`; if `content-length` header parses > cap → `skipped`; verify `content-type` startsWith `video/` (or empty/absent — IG sometimes omits it) else `error`; `mkdirSync`; stream `res.body` chunk-by-chunk to a `tempVideoPath` write stream, summing bytes — if the running total exceeds the cap, destroy the stream, `unlinkSync` the temp, return `skipped`; on clean end, `renameSync` temp → final and return `cached`; wrap in try/catch → `error` with `err.message`; `finally { inflight.delete(inflightKey); }`. Add `downloadVideo` to `module.exports`.

- [ ] **Step 4: Run → PASS** (6 tests). **Step 5: Commit** — `git commit -am "feat(video-cache): downloadVideo (streamed byte cap, url-scoped in-flight dedup)"`
...
**Selector [CX-1, CX-2] — retention on `posted_at`, freshness on `scraped_at`:**

```sql
SELECT id, shortcode, video_url FROM posts
WHERE video_url IS NOT NULL
  AND (posted_at >= $retentionCutoff OR posted_at IS NULL)          -- retention: bound storage
...
- `retentionCutoff` = now − `maxAgeDays` (default 30), formatted like the stored `posted_at`.
- `freshnessCutoff` = now − `freshnessDays` (default 14, matching the thumbnail sweep's recency window), formatted like the stored `scraped_at`.
- Both timestamps are ISO strings stored lexicographically-comparable; reuse the exact PG-`...Z` vs SQLite-`space` cutoff formatting from `sweepThumbnails` (`thumbnails.js:65-69`) for BOTH cutoffs.
- `concurrency` default **3**, `batchLimit` default **60**, jittered `delay` between items.
...
- [ ] **Step 1: Write the failing test (in-memory adapter + fake download)** [CX-14] — assert:
  ...
  5. **stale-URL race [CX-3]:** when the row's `video_url` is changed between selection and the status write (simulate by having the fake `download` mutate the row's URL, or by asserting the UPDATE carries `WHERE ... AND video_url = <selected>`), the status write does NOT overwrite — the row remains `'pending'`.
...
**Guarded clear [CX-10]:** select stale ids, then for each unlink the file (ignore ENOENT) and clear status with a predicate-guarded UPDATE so a post re-scraped into `'pending'` between select and update is not silently reset:

```sql
UPDATE posts SET video_cache_status = NULL, video_cached_at = NULL
WHERE id = $1 AND posted_at < $cutoff     -- re-check the window at write time
```

(A post 30+ days old is past retention regardless of a concurrent re-mark, so this is correct: if it's genuinely old it clears; if a race made it look recent, the guard skips it and the next sweep/prune settles it. The `unlinkSync` is idempotent via the ENOENT catch.)

- [ ] **Step 1: Write the failing test** [CX-14] — seed two posts (one 40 days old with a cached file present in fake fs, one 5 days old); run `pruneOldVideos({maxAgeDays:30})`; assert the old file was `unlinkSync`'d and its `video_cache_status` cleared, the recent one untouched; assert a missing file (ENOENT) does not throw.

- [ ] **Step 2: Run → FAIL. Step 3: implement** — SELECT `id` of posts with `posted_at < cutoff` AND `video_cache_status IS NOT NULL`; for each: `try { fs.unlinkSync(videoFilePath(post, videoDir)); } catch (e) { if (e.code !== 'ENOENT') ... }` then the guarded UPDATE above. Log `[Metric] video_prune deleted=N`. Add to exports.
...
- Produces: `GET /video/:id` — if the cached file exists → `res.sendFile` (Express handles `Range`→206/416, `Content-Type`, `Accept-Ranges`, and stream errors via the callback). If not cached but the post has a `video_url` → `302` to it (best-effort: plays a freshly-scraped-but-not-yet-swept video from its still-fresh URL; degrades to the shipped poster fallback if the URL is expired). Else `404`.
...
app.get('/video/:id', asyncHandler(async (req, res) => {
  ...
  if (post.video_url) return res.redirect(302, post.video_url); // fresh-but-unswept fallback
...
- [ ] **Step 2: Write the extracted-handler test [CX-12]** — `server/video-route.test.js`: unit-test the handler's branching by factoring the body into a small exported `serveVideo(post, { fs, videoDir, res })`-style function OR by asserting via fake `req`/`res`/`pool`/`fs` objects: (a) unknown id → 404; (b) cached file present → `res.sendFile` called with the computed path; (c) uncached + `video_url` present → `res.redirect(302, url)`; (d) uncached + no url → 404. (Auth itself is enforced by the `app.use('/video', requireAuth)` prefix, identical to `/thumb`; a note in the test documents that `/video` is listed among the `requireAuth` prefixes at `index.js:107`.)
...
- [ ] **Step 3: Post-scrape sweep.** After the existing `sweepThumbnails(...)` fire-and-forget, add:
...
- [ ] **Step 4: Verify** the full backend suite still passes (`cd server && node --test` → green; the scraper's existing tests must not regress). **Step 5: Commit** — `git commit -am "feat(video-cache): enqueue video caching on BOTH scrape insert paths + post-scrape sweep"`
...
**Interfaces:**
- Produces: the autoplay/tap-to-play `<video src>` points at `${API_URL}/video/${cardId}` (same-origin, cookie session flows automatically) instead of the raw `post.video_url`. The poster/`videoFailed` fallback (already shipped) stays: an uncached video whose source `video_url` is also dead will `302→403→onError→thumbnail`, exactly as now.

- [ ] **Step 1:** Compute `const videoSrc = cardId ? \`${API_URL}/video/${cardId}\` : post.video_url;` near `thumbnailSrc`, and set `<video src={videoSrc}>`. Keep `poster={thumbnailSrc}` and the `onError` → `videoFailed` handler.
...
- Incorporated: CX-1/2 (freshness=`scraped_at` + retention=`posted_at`, Task 4), CX-3 (URL-guarded UPDATE, Task 4), CX-5 (in-flight key includes URL, Task 3), CX-6 (streamed byte cap, Task 3), CX-8/9/15 (`res.sendFile`, drop range parser, Tasks 2+6), CX-10 (guarded prune clear, Task 5), CX-11 (both insert paths, Task 7), CX-12 (route/auth test + same-origin cookie note, Task 6 + Global Constraints), CX-13 (real-migration test, Task 1), CX-14 (the missing risky-path tests, Tasks 3/4/5/6).
- Rejected (rationale in the review log): CX-4 (cross-process `pending→downloading` lease — over-engineered for a single-instance Railway deploy; CX-3+CX-5 remove the real harms; matches the proven `thumbnails.js` pattern) and CX-7's *removal* of the 302 (kept narrowly — plays fresh-but-unswept videos, degrades to the already-shipped poster on expiry, and exposes no URL the authenticated same-origin client didn't already have).
