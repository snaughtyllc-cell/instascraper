...
export default function IdeaReel({ reel }) {
  ...
  return (
    <div className="flex-1 min-w-0">
        ...
        {playing && !failed ? (
          ...
          <>
            ...
            {failed && reelUrl ? (
              ...
              >
                <svg className="w-8 h-8 fill-white/90 drop-shadow-[0_2px_5px_rgba(0,0,0,0.5)]" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
              </button>
            )}
          </>
        )}
      </div>
      <div className="mt-1.5 flex items-center gap-1.5 text-[11px] font-semibold text-gray-400">
      ...
      {reel.account_handle && (
        <div className="text-[11px] font-semibold text-gold truncate">@{reel.account_handle}</div>
      )}
    </div>
  );
}
