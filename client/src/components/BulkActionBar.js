...
export default function BulkActionBar({ count, onTag, onArchive, onSetType, onSelectAll, onClear, actions = null }) {
      ...
      {hasCustomActions ? (
        actions.map((action) => (
          <button
            ...
            className={action.className || 'px-2.5 py-1.5 rounded-lg text-xs font-medium bg-gray-800 border border-gray-700 text-gray-300 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed'}
          >
            {action.label}
          </button>
        ))
